...
public abstract class Concourse {
	...
	/**
	 * Start and use an embedded Concourse database server, which requires the
	 * presence of a {@value #PREFS_FILE} configuration file in the working
	 * directory.
	 * 
	 * @return the handler for the embedded server
	 */
	public static EmbeddedServer useEmbeddedServer() {
		ConcourseConfiguration prefs = ConcourseConfiguration
				.fromFile(PREFS_FILE);
		return new EmbeddedServer(prefs);
	}

	// TODO useRemoteServer()
