...
public class ConcourseServer
        implements ConcourseRuntime, ConcourseServerMXBean {
    ...
    private static void chronologizeAtomic(String key, long record,
        ...
        if(latest.size() > history.size()) {
            for (int i = history.size(); i < latest.size(); ++i) {
                long timestamp = Iterables.get((Iterable<Long>) latest.keySet(),
                        i);
    ...
    /**
     * The server keeps a mapping of {@link AccessToken} objects to
     * {@link TransactionToken} objects and integers
     * to track the progression of the associated {@link Transaction}. The
     * integer will be nonzero if the given
     * TransactionToken and AccessToken are associated with a nested
     * Transaction. Currently, calling
     * {@link #stage(AccessToken, String)} during an ongoing transaction is a
     * no-op, and the Map is used to
    ...
    public Map<Long, Set<TObject>> chronologizeKeyRecordStartEnd(String key,
        ...
        Map<Long, Set<TObject>> base = chronologizeKeyRecord(key, record, creds,
                transaction, environment);
