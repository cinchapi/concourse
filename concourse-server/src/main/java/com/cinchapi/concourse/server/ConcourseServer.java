...
public class ConcourseServer extends BaseConcourseServer
        ...
        try {
            ...
            while (atomic == null || !atomic.commit()) {
                ...
                try {
                    ...
                    average = Operations.avgKeyRecordsAtomic(key, records, Time.NONE,
                            atomic);
                }
                catch (AtomicStateException e) {
                    average = 0;
        ...
        try {
            ...
            while (atomic == null || !atomic.commit()) {
                ...
                try {
                    ...
                    findAtomic(queue, stack, atomic);
                    Set<Long> records = stack.pop();
                    average = Operations.avgKeyRecordsAtomic(key, records, timestamp,
                            atomic);
                }
                catch (AtomicStateException e) {
                    average = 0;
    ...
    public TObject averageKeyRecord(String key, long record, AccessToken creds,
        ...
        while (atomic == null || !atomic.commit()) {
            atomic = store.startAtomicOperation();
            try {
                average = Operations.avgKeyRecordAtomic(key, record, Time.NONE, atomic);
            }
            catch (AtomicStateException e) {
                atomic = null;
                average = 0;
    ...
    public TObject averageKeyRecords(String key, List<Long> records,
        ...
        while (atomic == null || !atomic.commit()) {
            atomic = store.startAtomicOperation();
            try {
                average = Operations.avgKeyRecordsAtomic(key, records, Time.NONE, atomic);
            }
            catch (AtomicStateException e) {
                atomic = null;
                average = 0;
    ...
    public TObject averageKeyRecordsTime(String key, List<Long> records,
        ...
        while (atomic == null || !atomic.commit()) {
            atomic = store.startAtomicOperation();
            try {
                average = Operations.avgKeyRecordsAtomic(key, records, timestamp, atomic);
            }
            catch (AtomicStateException e) {
                atomic = null;
                average = 0;
    ...
    public TObject averageKeyRecordTime(String key, long record, long timestamp,
        ...
        while (atomic == null || !atomic.commit()) {
            atomic = store.startAtomicOperation();
            try {
                average = Operations.avgKeyRecordAtomic(key, record, timestamp, atomic);
            }
            catch (AtomicStateException e) {
                atomic = null;
                average = 0;
