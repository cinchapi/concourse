...
public class ConcourseServer extends BaseConcourseServer
    ...
    public long countKeyTimestr(String key, String timestamp,
        ...
        while (atomic == null || !atomic.commit()) {
            atomic = store.startAtomicOperation();
            try {
                count = Operations.countKeyAtomic(key,
    ...
    public long countKeyRecordTimestr(String key, long record,
        ...
        while (atomic == null || !atomic.commit()) {
            atomic = store.startAtomicOperation();
            try {
                count = Operations.countKeyRecordAtomic(key, record,
                        NaturalLanguage.parseMicros(timestamp), atomic);
            }
            catch (AtomicStateException e) {
                atomic = null;
    ...
    public long countKeyCriteria(String key, TCriteria criteria,
        ...
        while (atomic == null || !atomic.commit()) {
            ...
            try {
                ...
                Operations.findAtomic(queue, stack, atomic);
                Set<Long> records = stack.pop();
                count = Operations.countKeyRecordsAtomic(key, records, Time.NONE,
                        atomic);
            }
            catch (AtomicStateException e) {
                count = 0;
    ...
    public long countKeyCriteriaTime(String key, TCriteria criteria,
        ...
        while (atomic == null || !atomic.commit()) {
            ...
            try {
                ...
                Operations.findAtomic(queue, stack, atomic);
                Set<Long> records = stack.pop();
                count = Operations.countKeyRecordsAtomic(key, records, timestamp,
                        atomic);
            }
            catch (AtomicStateException e) {
                count = 0;
