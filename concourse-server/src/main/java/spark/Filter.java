...
/**
 * A Filter is built up by a path (for url-matching) and the implementation of
 * the 'handle' method.
 * When a request is made, if present, the matching routes 'handle' method is
 * invoked.
 *
 * @author Per Wendel
 */
public abstract class Filter extends AbstractRoute {
     ...
     * Constructor
     * 
     * @param path The filter path which is used for matching. (e.g. /hello,
     *            users/:name)
     */
     ...
     *            request
     * @param response The response object providing functionality for modifying
     *            the response
     */
