 ...
 * true in constructors, {@code ForkJoinPool}s may also be appropriate for use
 ...
 * {@code ForkJoinPool} may be constructed with a given target parallelism
 * level; by default, equal to the number of available processors. The pool
 * attempts to maintain enough active (or available) threads by dynamically
 * adding, suspending, or resuming internal worker threads, even if some tasks
 * are stalled waiting to join others. However, no such adjustments are
 * guaranteed in the face of blocked I/O or other unmanaged synchronization. The
 * nested {@link ManagedBlocker} interface enables extension of the kinds of
...
public class ForkJoinPool extends AbstractExecutorService {
    ...
    static final class WorkQueue {
        ...
        final boolean isEmpty() {
            ...
            int m, s;
            int n = base - (s = top);
            return (n >= 0 || (n == -1 && ((a = array) == null
                    || (m = a.length - 1) < 0 || U.getObject(a,
        ...
        final boolean isApparentlyUnblocked() {
            Thread wt;
            Thread.State s;
            return (eventCount >= 0 && (wt = owner) != null
    ...
    private void tryAddWorker() {
        long c;
        int u, e;
        while ((u = (int) ((c = ctl) >>> 32)) < 0 && (u & SHORT_SIGN) != 0
                && (e = (int) c) >= 0) {
    ...
    final void deregisterWorker(ForkJoinWorkerThread wt, Throwable ex) {
        ...
        if(wt != null && (w = wt.workQueue) != null) {
            ...
            do {}
            while (!U.compareAndSwapLong(this, STEALCOUNT, sc = stealCount, sc
                    + w.nsteals));
            if(((ps = plock) & PL_LOCK) != 0
                    || !U.compareAndSwapInt(this, PLOCK, ps, ps += PL_LOCK))
                ps = acquirePlock();
    ...
    private void fullExternalPush(ForkJoinTask<?> task) {
        ...
        for (Submitter z = submitters.get();;) {
            ...
            if((ps = plock) < 0)
                ...
                n |= n >>> 16;
                n = (n + 1) << 1;
                WorkQueue[] nws = ((ws = workQueues) == null || ws.length == 0 ? new WorkQueue[n]
                        : null);
                if(((ps = plock) & PL_LOCK) != 0
                        || !U.compareAndSwapInt(this, PLOCK, ps, ps += PL_LOCK))
                    ps = acquirePlock();
    ...
    private final int awaitWork(WorkQueue w, long c, int ec) {
        ...
        if((stat = w.qlock) >= 0 && w.eventCount == ec && ctl == c
            ...
            if(e < 0 || (d <= 0 && tryTerminate(false, false)))
                ...
                w.nsteals = 0;
                do {}
                while (!U.compareAndSwapLong(this, STEALCOUNT, sc = stealCount,
                        sc + ns));
            }
            else {
                ...
                if(pc != 0L) { // timed wait if last waiter
                    ...
                    parkTime = (dc < 0 ? FAST_IDLE_TIMEOUT : (dc + 1)
                            * IDLE_TIMEOUT);
    ...
    private final void helpRelease(long c, WorkQueue[] ws, WorkQueue w,
        ...
        if(w != null && w.eventCount < 0 && (e = (int) c) > 0 && ws != null
                && ws.length > (i = e & SMASK) && (v = ws[i]) != null
                && ctl == c) {
                        ...
                        if((b = v.base) - v.top < 0 && (a = v.array) != null) {
                            ...
                            if(v.base == b) {
                                ...
                                if(U.compareAndSwapObject(a, i, t, null)) {
                                    ...
                                    while (task.status >= 0 && joiner.top != jt
                                            && (t = joiner.pop()) != null);
        ...
        for (boolean active = true;;) {
            ...
            if((q = findNonEmptyStealQueue()) != null) {
                ...
                if(U.compareAndSwapLong(this, CTL, c, nc))
                    ...
                    && U.compareAndSwapLong(this, CTL, c,
                            ((c & ~AC_MASK) | ((c & AC_MASK) + AC_UNIT))))
                break;
        }
    }
            ...
            if(!now) { // check if idle & no tasks
                ...
                if((ws = workQueues) != null) {
                    for (int i = 0; i < ws.length; ++i) {
                        if((w = ws[i]) != null
                                && (!w.isEmpty() || ((i & 1) != 0 && w.eventCount >= 0))) {
                            signalWork(ws, w);
                            return false;
                        }
                    }
                    ...
                    if((ws = workQueues) != null) {
                        ...
                        while ((e = (int) (cc = ctl) & E_MASK) != 0
                            ...
                            long nc = ((long) (w.nextWait & E_MASK)
                                    | ((cc + AC_UNIT) & AC_MASK) | (cc & (TC_MASK | STOP_BIT)));
                            if(w.eventCount == (e | INT_SIGN)
    ...
    static WorkQueue commonSubmitterQueue() {
        ...
        WorkQueue[] ws;
        int m, r;
        return ((z = submitters.get()) != null && (p = common) != null
                && (ws = p.workQueues) != null && (m = ws.length - 1) >= 0) ? ws[m
                & z.seed & SQMASK]
                : null;
    }
    ...
    final int externalHelpComplete(CountedCompleter<?> task) {
        ...
        int s = 0;
        if(z != null && ws != null && (m = ws.length - 1) >= 0
                && (joiner = ws[(j = z.seed) & m & SQMASK]) != null
                && task != null) {
            int scans = m + m + 1;
            long c = 0L; // for stability check
            j |= 1; // poll odd queues
     ...
     * level, the {@linkplain #defaultForkJoinWorkerThreadFactory default thread
     * factory},
     * no UncaughtExceptionHandler, and non-async LIFO processing mode.
     * 
     ...
     *            establishes local first-in-first-out scheduling mode for
     *            forked
     ...
     * constructed; its run state is unaffected by attempts to {@link #shutdown}
     ...
     * {@link #commonPool()} never terminates until program shutdown, when
    ...
    private static ForkJoinPool makeCommonPool() {
        ...
        try { // ignore exceptions in accessing/parsing properties
            String pp = System
                    .getProperty("java.util.concurrent.ForkJoinPool.common.parallelism");
            String fp = System
