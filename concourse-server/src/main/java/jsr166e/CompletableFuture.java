 ...
 * 
 * <p>
 * Methods are available for adding dependents based on user-provided Functions,
 * Actions, or Runnables. The appropriate form to use depends on whether actions
 * require arguments and/or produce results. Completion of a dependent action
 * will trigger the completion of another CompletableFuture. Actions may also be
 * triggered after either or both the current and another CompletableFuture
 * complete. Multiple CompletableFutures may also be grouped as one using
 * {@link #anyOf(CompletableFuture...)} and {@link #allOf(CompletableFuture...)}.
 * 
 * <p>
 * CompletableFutures themselves do not execute asynchronously. However, actions
 * supplied for dependent completions of another CompletableFuture may do so,
 * depending on whether they are provided via one of the <em>async</em> methods
 * (that is, methods with names of the form <tt><var>xxx</var>Async</tt>). The
 * <em>async</em> methods provide a way to commence asynchronous processing of
 * an action using either a given {@link Executor} or by default the
 * {@link ForkJoinPool#commonPool()}. To simplify monitoring, debugging, and
 * tracking, all generated asynchronous tasks are instances of the marker
 * interface {@link AsynchronousCompletionTask}.
 * 
 * <p>
 * Actions supplied for dependent completions of <em>non-async</em> methods may
 * be performed by the thread that completes the current CompletableFuture, or
 * by any other caller of these methods. There are no guarantees about the order
 * of processing completions unless constrained by these methods.
 * 
 * <p>
 * Since (unlike {@link FutureTask}) this class has no direct control over the
 * computation that causes it to be completed, cancellation is treated as just
 * another form of exceptional completion. Method {@link #cancel cancel} has the
 * same effect as {@code completeExceptionally(new CancellationException())}.
 * 
 * <p>
 * Upon exceptional completion (including cancellation), or when a completion
 * entails an additional computation which terminates abruptly with an
 * (unchecked) exception or error, then all of their dependent completions (and
 * their dependents in turn) generally act as {@code completeExceptionally} with
 * a {@link CompletionException} holding that exception as its cause. However,
 * the {@link #exceptionally exceptionally} and {@link #handle handle}
 * completions <em>are</em> able to handle exceptional completions of the
 * CompletableFutures they depend on.
 * 
 * <p>
 ...
 * CompletionException. However, in these cases, methods {@link #join()} and
 * {@link #getNow} throw the CompletionException, which simplifies usage.
 * 
 * <p>
 * Arguments used to pass a completion result (that is, for parameters of type
 * {@code T}) may be null, but passing a null value for any other parameter will
...
public class CompletableFuture<T> implements Future<T> {
    ...
    final void internalComplete(T v, Throwable ex) {
        if(result == null)
            UNSAFE.compareAndSwapObject(this, RESULT, null,
                    (ex == null) ? (v == null) ? NIL : v : new AltResult(
     ...
     * @return raw result
     */
    private Object timedAwaitDone(long nanos) throws InterruptedException,
            TimeoutException {
        WaitNode q = null;
        boolean queued = false;
        for (Object r;;) {
    ...
    abstract static class Async extends ForkJoinTask<Void> implements
            Runnable,
    ...
    static final class AsyncAcceptBoth<T, U> extends Async {
        ...
        AsyncAcceptBoth(T arg1, U arg2, BiAction<? super T, ? super U> fn,
    ...
    static final class AsyncCompose<T, U> extends Async {
        ...
        AsyncCompose(T arg, Fun<? super T, CompletableFuture<U>> fn,
                CompletableFuture<U> dst) {
            this.arg = arg;
            this.fn = fn;
            this.dst = dst;
    ...
    static final class ThenApply<T, U> extends Completion {
        ...
        ThenApply(CompletableFuture<? extends T> src,
                Fun<? super T, ? extends U> fn, CompletableFuture<U> dst,
                Executor executor) {
            this.src = src;
            this.fn = fn;
            this.dst = dst;
    ...
    static final class ThenAccept<T> extends Completion {
        ...
        ThenAccept(CompletableFuture<? extends T> src, Action<? super T> fn,
    ...
    static final class ThenRun extends Completion {
        ...
        ThenRun(CompletableFuture<?> src, Runnable fn,
                CompletableFuture<Void> dst, Executor executor) {
            this.src = src;
            this.fn = fn;
            this.dst = dst;
    ...
    static final class ThenCombine<T, U, V> extends Completion {
        ...
        public final void run() {
            ...
            if((dst = this.dst) != null && (fn = this.fn) != null
                    && (a = this.src) != null && (r = a.result) != null
                    && (b = this.snd) != null && (s = b.result) != null
                    && compareAndSet(0, 1)) {
                if(r instanceof AltResult) {
                    ex = ((AltResult) r).ex;
                    t = null;
    ...
    static final class ThenAcceptBoth<T, U> extends Completion {
        ...
        ThenAcceptBoth(CompletableFuture<? extends T> src,
                CompletableFuture<? extends U> snd,
                BiAction<? super T, ? super U> fn, CompletableFuture<Void> dst,
                Executor executor) {
            this.src = src;
            this.snd = snd;
            this.fn = fn;
    ...
    static final class RunAfterBoth extends Completion {
        ...
        RunAfterBoth(CompletableFuture<?> src, CompletableFuture<?> snd,
                Runnable fn, CompletableFuture<Void> dst, Executor executor) {
            this.src = src;
            this.snd = snd;
            this.fn = fn;
            this.dst = dst;
    ...
    static final class AndCompletion extends Completion {
        ...
        AndCompletion(CompletableFuture<?> src, CompletableFuture<?> snd,
                CompletableFuture<Void> dst) {
            this.src = src;
            this.snd = snd;
            this.dst = dst;
        ...
        public final void run() {
            ...
            Object r, s;
            Throwable ex;
            if((dst = this.dst) != null && (a = this.src) != null
                    && (r = a.result) != null && (b = this.snd) != null
                    && (s = b.result) != null && compareAndSet(0, 1)) {
                if(r instanceof AltResult)
                    ex = ((AltResult) r).ex;
                else
                    ex = null;
    ...
    static final class ApplyToEither<T, U> extends Completion {
        ...
        public final void run() {
            ...
            if((dst = this.dst) != null
                    && (fn = this.fn) != null
                    && (((a = this.src) != null && (r = a.result) != null) || ((b = this.snd) != null && (r = b.result) != null))
                    && compareAndSet(0, 1)) {
                if(r instanceof AltResult) {
                    ex = ((AltResult) r).ex;
                    t = null;
    ...
    static final class AcceptEither<T> extends Completion {
        ...
        AcceptEither(CompletableFuture<? extends T> src,
                CompletableFuture<? extends T> snd, Action<? super T> fn,
                CompletableFuture<Void> dst, Executor executor) {
            this.src = src;
            this.snd = snd;
            this.fn = fn;
    ...
    static final class RunAfterEither extends Completion {
        ...
        RunAfterEither(CompletableFuture<?> src, CompletableFuture<?> snd,
                Runnable fn, CompletableFuture<Void> dst, Executor executor) {
            this.src = src;
            this.snd = snd;
            this.fn = fn;
    ...
    static final class OrCompletion extends Completion {
        ...
        OrCompletion(CompletableFuture<?> src, CompletableFuture<?> snd,
    ...
    static final class ExceptionCompletion<T> extends Completion {
        ...
        ExceptionCompletion(CompletableFuture<? extends T> src,
                Fun<? super Throwable, ? extends T> fn, CompletableFuture<T> dst) {
            this.src = src;
            this.fn = fn;
            this.dst = dst;
        }
                ...
                if(c != null) {
                    ...
                    if((s = c.result) == null) {
                        ...
                        while ((s = c.result) == null) {
                            if(UNSAFE.compareAndSwapObject(c, COMPLETIONS,
                                    p.next = c.completions, p))
                                break;
                        }
                    }
    ...
    public static <U> CompletableFuture<U> supplyAsync(Generator<U> supplier) {
        ...
        ForkJoinPool.commonPool().execute(
                (ForkJoinTask<?>) new AsyncSupply<U>(supplier, f));
        return f;
    }
    ...
    public static CompletableFuture<Void> runAsync(Runnable runnable) {
        ...
        ForkJoinPool.commonPool().execute(
                (ForkJoinTask<?>) new AsyncRun(runnable, f));
        return f;
    }
     ...
     * 
     * <p>
     * If this CompletableFuture completes exceptionally, or the supplied
     * function throws an exception, then the returned CompletableFuture
     * completes exceptionally with a CompletionException holding the exception
     * as its cause.
     * 
     ...
     */
    public <U> CompletableFuture<U> thenApplyAsync(
            Fun<? super T, ? extends U> fn, Executor executor) {
        if(executor == null)
            throw new NullPointerException();
    ...
    private <U> CompletableFuture<U> doThenApply(
            Fun<? super T, ? extends U> fn, Executor e) {
        if(fn == null)
            throw new NullPointerException();
        ...
        Object r;
        if((r = result) == null) {
            CompletionNode p = new CompletionNode(d = new ThenApply<T, U>(this,
                    fn, dst, e));
            while ((r = result) == null) {
     ...
     * 
     * <p>
     * If this CompletableFuture completes exceptionally, or the supplied action
     * throws an exception, then the returned CompletableFuture completes
     * exceptionally with a CompletionException holding the exception as its
     * cause.
     * 
    ...
    private CompletableFuture<Void> doThenAccept(Action<? super T> fn,
        ...
        Object r;
        if((r = result) == null) {
            CompletionNode p = new CompletionNode(d = new ThenAccept<T>(this,
                    fn, dst, e));
            while ((r = result) == null) {
                if(UNSAFE.compareAndSwapObject(this, COMPLETIONS,
                        p.next = completions, p))
    ...
    private CompletableFuture<Void> doThenRun(Runnable action, Executor e) {
        ...
        Object r;
        if((r = result) == null) {
            CompletionNode p = new CompletionNode(d = new ThenRun(this, action,
                    dst, e));
            while ((r = result) == null) {
                if(UNSAFE.compareAndSwapObject(this, COMPLETIONS,
                        p.next = completions, p))
     ...
     * 
     * <p>
     * If this and/or the other CompletableFuture complete exceptionally, or the
    ...
    public <U, V> CompletableFuture<V> thenCombineAsync(
            CompletableFuture<? extends U> other,
            BiFun<? super T, ? super U, ? extends V> fn, Executor executor) {
        if(executor == null)
            throw new NullPointerException();
    ...
    private <U, V> CompletableFuture<V> doThenCombine(
            CompletableFuture<? extends U> other,
            BiFun<? super T, ? super U, ? extends V> fn, Executor e) {
        ...
        if((r = result) == null || (s = other.result) == null) {
            ...
            while ((r == null && (r = result) == null)
                ...
                if(q != null) {
                    if(s != null
                            || UNSAFE.compareAndSwapObject(other, COMPLETIONS,
                                    q.next = other.completions, q))
                        break;
                }
                else if(r != null
                        || UNSAFE.compareAndSwapObject(this, COMPLETIONS,
                                p.next = completions, p)) {
                    if(s != null)
                        break;
    ...
    public <U> CompletableFuture<Void> thenAcceptBothAsync(
            CompletableFuture<? extends U> other,
            BiAction<? super T, ? super U> block, Executor executor) {
        if(executor == null)
            throw new NullPointerException();
    ...
    private <U> CompletableFuture<Void> doThenAcceptBoth(
            CompletableFuture<? extends U> other,
            BiAction<? super T, ? super U> fn, Executor e) {
        if(other == null || fn == null)
            throw new NullPointerException();
        CompletableFuture<Void> dst = new CompletableFuture<Void>();
    ...
    public CompletableFuture<Void> runAfterBothAsync(
            CompletableFuture<?> other, Runnable action) {
     ...
     */
    public CompletableFuture<Void> runAfterBothAsync(
            CompletableFuture<?> other, Runnable action, Executor executor) {
        if(executor == null)
            throw new NullPointerException();
     ...
     * 
     * <p>
     * If this and/or the other CompletableFuture complete exceptionally, then
     * the returned CompletableFuture may also do so, with a CompletionException
     * holding one of these exceptions as its cause. No guarantees are made
     * about which result or exception is used in the returned
     * CompletableFuture. If the supplied function throws an exception, then the
    ...
    public <U> CompletableFuture<U> applyToEither(
            CompletableFuture<? extends T> other, Fun<? super T, U> fn) {
    ...
    private <U> CompletableFuture<U> doApplyToEither(
        ...
        if((r = result) == null && (r = other.result) == null) {
            ...
            while ((r = result) == null && (r = other.result) == null) {
                if(q != null) {
                    if(UNSAFE.compareAndSwapObject(other, COMPLETIONS,
                            q.next = other.completions, q))
                        break;
                }
                else if(UNSAFE.compareAndSwapObject(this, COMPLETIONS,
                        p.next = completions, p))
                    q = new CompletionNode(d);
            }
        }
     ...
     * holding one of these exceptions as its cause. No guarantees are made
     * about which result or exception is used in the returned
     * CompletableFuture. If the supplied action throws an exception, then the
     * returned CompletableFuture completes exceptionally with a
     * CompletionException holding the exception as its cause.
     * 
     * @param other the other CompletableFuture
    ...
    public CompletableFuture<Void> acceptEither(
            CompletableFuture<? extends T> other, Action<? super T> block) {
    ...
    private CompletableFuture<Void> doAcceptEither(
            CompletableFuture<? extends T> other, Action<? super T> fn,
            Executor e) {
        if(other == null || fn == null)
            throw new NullPointerException();
        CompletableFuture<Void> dst = new CompletableFuture<Void>();
    ...
    private CompletableFuture<Void> doRunAfterEither(
            CompletableFuture<?> other, Runnable action, Executor e) {
        if(other == null || action == null)
            throw new NullPointerException();
        CompletableFuture<Void> dst = new CompletableFuture<Void>();
     ...
     * exception as its cause. Similarly, if the computed CompletableFuture
     * completes exceptionally, then so does the returned CompletableFuture.
     * 
     ...
     * {@link ForkJoinPool#commonPool()} by the given function of the result
     ...
     */
    public <U> CompletableFuture<U> thenComposeAsync(
            Fun<? super T, CompletableFuture<U>> fn, Executor executor) {
        if(executor == null)
            throw new NullPointerException();
    ...
    private <U> CompletableFuture<U> doThenCompose(
            Fun<? super T, CompletableFuture<U>> fn, Executor e) {
        if(fn == null)
            throw new NullPointerException();
        ...
        if((r = result) == null) {
            ...
            CompletionNode p = new CompletionNode(d = new ThenCompose<T, U>(
                    this, fn, dst, e));
            while ((r = result) == null) {
                if(UNSAFE.compareAndSwapObject(this, COMPLETIONS,
                        p.next = completions, p))
        ...
        if(len > 1)
            ...
            if(len == 0)
                ...
                while ((r = f.result) == null) {
                    ...
                    else if(UNSAFE.compareAndSwapObject(f, COMPLETIONS,
                            p.next = f.completions, p))
                        break;
                }
                if(r != null && (d == null || d.compareAndSet(0, 1)))
                    dst.internalComplete(null,
                            (r instanceof AltResult) ? ((AltResult) r).ex
                                    : null);
                f.helpPostComplete();
            }
            return dst;
    ...
    private static CompletableFuture<Void> allTree(CompletableFuture<?>[] cfs,
        ...
        while ((r = fst.result) == null || (s = snd.result) == null) {
            if(d == null)
                ...
                if(UNSAFE.compareAndSwapObject(fst, COMPLETIONS,
                        p.next = fst.completions, p))
                    q = new CompletionNode(d);
            }
            else if(UNSAFE.compareAndSwapObject(snd, COMPLETIONS,
                    q.next = snd.completions, q))
                break;
        }
     ...
     */
    private static CompletableFuture<Object> anyTree(
    ...
    public boolean cancel(boolean mayInterruptIfRunning) {
        boolean cancelled = (result == null)
                && UNSAFE.compareAndSwapObject(this, RESULT, null,
                        new AltResult(new CancellationException()));
        postComplete();
     ...
     * related methods to throw the given exception, whether or
     ...
     * String {@code "Completed Normally"} or the String
     * {@code "Completed Exceptionally"}, or the String {@code "Not
    ...
    public String toString() {
        ...
        return super.toString()
                + ((r == null) ? (((count = getNumberOfDependents()) == 0) ? "[Not completed]"
    ...
    static {
        try {
            ...
            COMPLETIONS = UNSAFE.objectFieldOffset(k
