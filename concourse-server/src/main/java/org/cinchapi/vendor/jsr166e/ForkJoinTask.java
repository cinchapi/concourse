 ...
 * processing should minimally implement {@code protected} methods {@link #exec}, {@link #setRawResult}, and {@link #getRawResult}, while also introducing an
...
public abstract class ForkJoinTask<V> implements Future<V>, Serializable {
    ...
    private int doJoin() {
        ...
        return (s = status) < 0 ? s
                : ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread) ? (w = (wt = (ForkJoinWorkerThread) t).workQueue)
                        .tryUnpush(this) && (s = doExec()) < 0 ? s : wt.pool
                        .awaitJoin(w, this) : externalAwaitDone();
    }
     ...
     * related methods, or a call to {@link #isDone} returning {@code true}.
     * 
     ...
     * abnormal completion results in {@code RuntimeException} or {@code Error},
     * not {@code ExecutionException}, and that
     * interrupts of the calling thread do <em>not</em> cause the
     * method to abruptly return by throwing {@code InterruptedException}.
     * 
     ...
     * status of each task may be obtained using {@link #getException()} and
     * related methods to check if they have been
     ...
     * cancelled, causes it to throw the given exception upon {@code join} and
     * related operations. This method may be used
     ...
     *            exception
     *            thrown will be a {@code RuntimeException} with cause
     *            {@code ex}.
     */
     ...
     * recent value established by {@link #setRawResult} (or {@code null} by
     * default) will be returned as the result of subsequent
                ...
                while ((s = status) >= 0) {
                    if(w != null && w.qlock < 0)
                        ...
                        if((ms = TimeUnit.NANOSECONDS.toMillis(ns)) > 0L
                                && U.compareAndSwapInt(this, STATUS, s, s
                                        | SIGNAL)) {
                            synchronized (this) {
                                if(status >= 0) {
                                    try {
    ...
    public static ForkJoinPool getPool() {
        Thread t = Thread.currentThread();
        return (t instanceof ForkJoinWorkerThread) ? ((ForkJoinWorkerThread) t).pool
                : null;
    }
     ...
     * {@code if (task.compareAndSetForkJoinTaskTag((short)0, (short)1))} before
     * processing, otherwise exiting because the node has
    ...
    public final boolean compareAndSetForkJoinTaskTag(short e, short tag) {
        for (int s;;) {
            if((short) (s = status) != e)
                return false;
            if(U.compareAndSwapInt(this, STATUS, s, (s & ~SMASK)
                    | (tag & SMASK)))
                return true;
        }
    }
     ...
     */
    static final class AdaptedRunnableAction extends ForkJoinTask<Void> implements
            RunnableFuture<Void> {
        final Runnable runnable;
     ...
     * of the given {@code Runnable} as its action, and returns
     ...
     * of the given {@code Callable} as its action, and returns
