 ...
 * @apiNote
 *          <p>
 *          Spliterators, like {@code Iterators}s, are for traversing the
 *          elements of
 *          a source. The {@code Spliterator} API was designed to support
 *          efficient
 *          parallel traversal in addition to sequential traversal, by
 *          supporting
 *          decomposition as well as single-element iteration. In addition, the
 *          protocol for accessing elements via a Spliterator is designed to
 *          impose
 ...
 *          occur if
 *          the source is structurally interfered with (elements added,
 *          replaced, or
 *          removed) between the time that the Spliterator binds to its data
 *          source and
 *          the end of traversal. For example, such interference will produce
 *          arbitrary,
 *          non-deterministic results when using the {@code java.util.stream}
 *          framework.
 *
 *          <p>
 ...
 *          is a concurrent source. A Spliterator created from the source
 *          reports a
 ...
 *          {@link ConcurrentModificationException}. For example,
 *          {@link ArrayList},
 ...
 *          parallel
 *          computation, here is one way to implement an associated parallel
 *          forEach,
 *          that illustrates the primary usage idiom of splitting off subtasks
 *          until
 ...
 *           {@code org.openjdk.java.util.stream.tripwire}
...
public interface Spliterator<T> {
     ...
     *
     * @apiNote
     *          Even an inexact estimate is often useful and inexpensive to
     *          compute.
     *          For example, a sub-spliterator of an approximately balanced
     *          binary tree
     *          may return a value that estimates the number of elements to be
     *          half of
     ...
     *           {@code estimateSize()}
     *           if the Spliterator reports a characteristic of {@code SIZED},
     *           and
     *           {@code -1} otherwise.
     *
     ...
     *          may differ from the characteristics after splitting. For
     *          specific
     ...
     *
     * @implSpec
     *           The default implementation returns true if the corresponding
     *           bits
     ...
     * if the source is not {@code SORTED}, throws
     ...
     *          but not
     *          {@code SUBSIZED}, since it is common to know the size of the
     *          entire tree
