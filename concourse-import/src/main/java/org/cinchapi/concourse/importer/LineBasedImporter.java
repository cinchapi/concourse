...
public abstract class LineBasedImporter extends JsonImporter {
	...
	public final Set<Long> importFile(String file, @Nullable String resolveKey) {
		...
		Set<Long> records = upsert ? upsertJsonString(array.toString()) : importJsonString(array.toString());
		return records;
	}
	...
	private final String[] parseKeys(String line) {
		...
		if (useOptimizedSplitPath) {
			...
			keys = Strings.splitStringByDelimiterButRespectQuotes(line, delimiter());
	...
	/**
	 * Validate if given file is csv
	 * 
	 * @param line this is the first line of the given file
	 *            
	 */
