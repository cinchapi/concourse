...
public class ResolvableLinkTest extends ConcourseIntegrationTest {
    ...
    public void testInsertRemoteResolvableLink() {
        ...
        json.addProperty("friends",
                Convert.stringToResolvableLinkInstruction("age > " + value));
    ...
    public void testInsertLocalResolvableLink() {
        ...
        client.insert(json);
        Map<Long, Set<Object>> data = client.select("similar_to",
                "name = Jeff");
        ...
        for (long record : records) {
            Assert.assertTrue(
                    client.find("similar_to lnk2 " + record).isEmpty());
        }
    }
    ...
    public void testInsertRemoteResolvableLinkInExistingRecord() {
        ...
        JsonObject json = new JsonObject();
        json.addProperty("co_workers", Convert
                .stringToResolvableLinkInstruction("company = Cinchapi"));
        client.insert(json.toString(),
                Lists.newArrayList(record1, record2, record3));
        Map<Long, Set<Object>> data = client.select("co_workers",
                Lists.newArrayList(record1, record2, record3));
        for (Set<Object> set : data.values()) {
            Assert.assertEquals(2, set.size());
        }
