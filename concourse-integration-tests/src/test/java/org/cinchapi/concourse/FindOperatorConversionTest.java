...
public class FindOperatorConversionTest extends ConcourseIntegrationTest {
	...
	public void testLessThanOperatorConversion(){
		Assert.assertEquals(client.find("age", Operator.LESS_THAN, 25), client.find("age", "<", 25));
	}

	@Test
	public void testLessThanOrEqualToOperatorConversion(){
		Assert.assertEquals(client.find("age", Operator.LESS_THAN_OR_EQUALS, 2), client.find("age", "<=", 2));
	}

	@Test
	public void testGreaterThanOperatorConversion(){
		Assert.assertEquals(client.find("age", Operator.GREATER_THAN, 50), client.find("age", ">", 50));
	}

	@Test
	public void testGreaterThanOrEqualOperatorToConversion(){
		Assert.assertEquals(client.find("age", Operator.GREATER_THAN_OR_EQUALS, 99), client.find("age", ">=", 99));
	}

	@Test
	public void testEqualsOperatorConversion(){
		Assert.assertEquals(client.find("age", Operator.EQUALS, 15), client.find("age", "=", 15));
	}

	@Test
	public void testNotEqualsOperatorConversion(){
		Assert.assertEquals(client.find("age", Operator.NOT_EQUALS, 19), client.find("age", "!=", 19));
	}

	@Test
	public void testBetweenOperatorConversion(){
		Assert.assertEquals(client.find("age", Operator.BETWEEN, 40, 50), client.find("age", "><", 40, 50));
	}
}
