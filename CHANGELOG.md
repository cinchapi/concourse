...
* Streamlined the logic for reads that have a combination of `time`, `order` and `page` parameters by adding more intelligent heuristics for determining the most efficient code path. 
	* For example, a read that only has `time` and `page` parameters (e.g., no `order`) does not need to be performed atomically. Previously, those reads converged into an atomic code path, but now a separate code path exists so those reads can be more performant.
	* As another example, any read that performs pagination without sorting (e.g., has a `page`, but not `order` parameter) can be executed such that the pagination is applied to the lookup records (either those directly supplied as a `records` parameter or indirectly and resolved through a `ccl` statement or `criteria`) BEFORE the data is selected from the storage engine. Previously, pagination was always applied to last, so cases like this cause more data that necessary to be retrieved.
