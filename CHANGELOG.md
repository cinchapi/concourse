...
#### Version 0.9.7 (TBD)
* Fixed a bug that caused Concourse Server to start an environment's storage engine in a partially or wholly unreadable state if the Engine partially completed a block sync while Concourse Server was going through its shutdown routine. In this scenario, the partially written block is malformed and should not be processed by the Engine. The data contained in the malformed block was not lost (it was still contained in the Buffer). While the malformed block files can be safely deleted, the Engine will simply ignore them if they are encountered upon initialization.
