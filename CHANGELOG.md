...
* Improved user-defined `transactions` by detecting when an attempt is made to atomically commit multiple Writes that undo each other (e.g. `ADD name as jeff in 1`, `REMOVE name as jeff in 1`, `ADD name as jeff in 1`) and only committing at most one equal Write that is required to attain the intended state. For example, in the previous example, only 1 write for `ADD name as jeff in 1` would be committed.
